...
import com.embabel.example.dogfood.finance.DumbChatService
import com.embabel.example.dogfood.finance.FinanceAnalystProperties
import com.embabel.example.dogfood.finance.TradingAnalyst
...
class FinanceAgentIntegrationTest {

    @Test
    fun `agent runs`() {
        val mockEnvironment = mockk<Environment>()
        val mockOperationContext = mockk<OperationContext>()
        val chatService = mockk<DumbChatService>()
        every { chatService.promptUser(any()) } returns "Moderate"
        val agent: Agent = AgentMetadataReader().createAgentMetadata(
            TradingAnalyst(FinanceAnalystProperties(), chatService),
        ...
        val result = ap.runAgentWithInput(
            ...
            input = UserInput("I want to invest tesla"),
        )
